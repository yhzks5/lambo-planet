...
$(function() {
	...
	$('.switch-systems').click(function () {
		...
		$.cookie('lambo-upms-systemid', systemid);
		$.cookie('lambo-upms-systemname', systemname);
		$.cookie('lambo-upms-systemtitle', systemtitle);
	});
	// 显示cookie菜单
	var systemid = $.cookie('lambo-upms-systemid') || 1;
	var systemname = $.cookie('lambo-upms-systemname') || 'lambo-upms-server';
	var systemtitle = $.cookie('lambo-upms-systemtitle') || '权限管理系统';
	$('.system_menus').hide(0, function () {
		$('.system_' + systemid).show();
	});
	$('body').attr('id', systemname);
